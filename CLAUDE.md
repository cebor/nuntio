...
| Crate | Role |
|---|---|
| `crates/nuntio` | Binary: winit event loop (`app.rs`), window state and layout (`window.rs`), tabs (`tabs.rs`), split tree (`pane_tree.rs`), key and mouse encoding (`input.rs`, `mouse.rs`), shortcuts (`actions.rs`), UI overlays (`tab_bar.rs`, `search_bar.rs`, `banner.rs`) |
